package com.example.sad.lineChart.components
...
import com.example.sad.lineChart.utils.clickedOnThisPoint
import com.example.sad.lineChart.utils.formatToThousandsMillionsBillions
...
internal fun DrawScope.drawQuarticLineWithShadow(
    ...
    if (line.lineShadow && !specialChart) {
        ...
        clipRect(right = size.width * animatedProgress.value, bottom = size.height - 2*spacingY.value) {
            drawPath(
                path = fillPath,
                brush = Brush.verticalGradient(
                    colors = listOf(
                        line.lineColor.copy(alpha = .3f), line.lineColor.copy(alpha = .05f)
                    ), endY = (size.height.toDp()-2*spacingY).toPx()
                )
            )
        }
    }
